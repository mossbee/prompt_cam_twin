...
def get_dog(params, mode='trainval_combined'):
    params.class_num = 120
    ...
    if mode == 'trainval_combined':
        train_data_list = f'data/annotations/dog/{params.data}_combine.txt'

        if not os.path.exists(train_data_list):
            create_annotation_file(params.data_path, ['train'],train_data_list)
        return DogDataset(params.data_path, train_data_list, transform_train)

    elif mode == 'test':
        test_data_list = f'data/annotations/dog/test.txt'
        if not os.path.exists(test_data_list):
            create_annotation_file(params.data_path,['val'],test_data_list)
        return DogDataset(params.data_path, test_data_list, transform_val)
    else:
